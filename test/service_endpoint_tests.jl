...
@testset "Service endpoint (ROUTER concurrency rework)" begin
    ...
    @testset "multipart framing round-trips REQ↔ROUTER" begin
        ...
        ctx = ZMQt.Context()
        router = ZMQt.Socket(ctx, ZMQt.ROUTER)
        router.rcvtimeo = 2000
        router.linger = 0
        ZMQt.bind(router, "ipc://$path")
        req = ZMQt.Socket(ctx, ZMQt.REQ)
        req.rcvtimeo = 2000
        req.linger = 0
        ...
        try
            ...
            Kaimon._send_multipart(
                router,
                Vector{UInt8}[parts[1], UInt8[], Vector{UInt8}("pong")],
            )
    ...
    @testset "two REQ clients are routed back by identity" begin
        ...
        router.linger = 0
        ZMQt.bind(router, "ipc://$path")
        a = ZMQt.Socket(ctx, ZMQt.REQ)
        a.rcvtimeo = 2000
        a.linger = 0
        ZMQt.connect(a, "ipc://$path")
        b = ZMQt.Socket(ctx, ZMQt.REQ)
        b.rcvtimeo = 2000
        b.linger = 0
        ZMQt.connect(b, "ipc://$path")
        try
            ...
            for _ = 1:2
                ...
                Kaimon._send_multipart(
                    router,
                    Vector{UInt8}[p[1], UInt8[], Vector{UInt8}("re:" * tag)],
                )
            end
