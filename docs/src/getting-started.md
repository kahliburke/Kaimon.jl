...
Add the lightweight gate package to your project and call `serve()` in any
running REPL:
...
# Uses the lightweight KaimonGate package.
try
    using Revise
catch e
...
It then calls `KaimonGate.serve()` to register the REPL with Kaimon. This uses
the lightweight `KaimonGate` package (ZMQ + stdlib only), installed once into
your global environment, so every session can connect without pulling in the
full Kaimon dependency tree.

Both are wrapped in `try/catch` so Julia starts normally even if KaimonGate isn't installed or the server isn't running.
:::

!!! note "Upgrading from an older setup"
    If you previously connected via the full `Kaimon` package (a `using Kaimon; Gate.serve()` snippet), the `kaimon` CLI offers a one-time opt-in update on startup that installs the lightweight `KaimonGate` globally, removes the heavyweight `Kaimon` from your global environment, and migrates your `startup.jl` snippet to the form above.
...
KaimonGate.serve(tools=[KaimonGate.GateTool("summarize", summarize)])
```
