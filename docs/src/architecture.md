...
### Service Endpoint (Reverse Calls)

The Gate provides a service endpoint that allows gate sessions (including extensions) to call back into Kaimon's MCP tools. This uses a ZMQ REQ/REP socket at `~/.cache/kaimon/sock/kaimon-service.sock`:
...
The session manager (`src/session_manager.jl`) handles spawning and monitoring managed sessions. Each `ManagedSession` tracks the PTY subprocess, connection status, and session key. Sessions are spawned via the `start_session` MCP tool for projects in the allowed-projects list (`~/.config/kaimon/projects.json`).

### Extension Manager

The extension manager (`src/extension_manager.jl`) handles the lifecycle of `ManagedExtension` instances. It spawns extension subprocesses, monitors their health, and auto-restarts crashed extensions with exponential backoff. Extensions are configured via `~/.config/kaimon/extensions.json`. See [Extensions](extensions.md) for details.
...
Security is configured in `~/.config/kaimon/config.json`. The configuration defines:
...
| Path | Purpose |
|------|---------|
| `~/.config/kaimon/config.json` | Global security and editor configuration |
...
| `~/.config/kaimon/projects.json` | Allowed projects for managed sessions |
| `~/.config/kaimon/extensions.json` | Extension registry |
| `~/.cache/kaimon/extensions/*.log` | Extension process logs |
| `~/.cache/kaimon/sessions/*.log` | Managed session logs |
| `~/.cache/kaimon/` | Server log, database, and other operational files |
