...
grep_code_tool = @mcp_tool(
    :grep_code,
    "Find an EXACT PATTERN over files when you ALREADY HAVE the token — a symbol name, a call site, a string, a TODO, or a regex. (Just EXPLORING, or can only DESCRIBE what the code does? Use search_code instead — grep only finds the literal text you type, so guessing a symbol name and grepping it is the classic miss; it's blind to synonyms, indirection, and the code you didn't know to name.) Runs a real regex over the live working tree (no stale index), scoped to the bound project by default (narrow with path=/file=/glob=, all resolved relative to the PROJECT ROOT — a glob is anchored to the repo root like `cd repo && rg -g`, not to path=, so don't repeat a path= prefix inside a glob; a bare basename glob matches at any depth), and returns file:line WITH the enclosing function/struct for each code hit — `grep_code(pattern=\"_eval_with_capture\")` is the right move over shell grep/rg/find, which miss the enclosing symbol and aren't repo-scoped. By default it respects .gitignore (clean code search); pass no_ignore=true to also search logs and generated/gitignored/hidden files, so it covers the same ground as shell grep. Add an optional natural-language `query` to RANK the matching files by relevance and auto-expand context around relevant hits. Reads are confined to the bound project and your declared workspace roots; a path/file outside that scope prompts the user to approve it (once, or \"always\" to remember it), so don't assume out-of-project reads will just work. Flags: ignore_case, word (whole-word), fixed (literal, not regex), no_ignore (include ignored/hidden files).",
    _GREP_CODE_PARAMS,
