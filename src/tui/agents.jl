...
function _agent_status_icon(status::Symbol, last_activity::Real, tick::Int)
    breath(c) = Style(
        fg = brighten(c, pulse(tick; period = 70, lo = 0.0, hi = 1.0) * 0.4),
        bold = true,
    )
    @match status begin
        :working => (
            (time() - last_activity) > _AGENT_STALL_SECS ? ("⬤", breath(_AGENT_AMBER)) :
            ("⬤", breath(_AGENT_BLUE))
        )
...
function _ago(t::Real)
    d = max(0.0, time() - t)
    d < 60 ? "$(round(Int, d))s" :
    d < 3600 ? "$(round(Int, d / 60))m" : "$(round(Int, d / 3600))h"
end
...
function _view_agents_list(m::KaimonModel, area::Rect, buf::Buffer, ags)
    ...
    if isempty(ags)
        ...
        set_string!(
            ...
            inner.x + 1,
            inner.y + 1,
            "Open one with the agent_open MCP tool.",
            tstyle(:text_dim),
        )
        return
    end
    ...
    items = ListItem[]
    for a in ags
        icon, istyle = _agent_status_icon(
            Symbol(get(a, "status", "?")),
            Float64(get(a, "last_activity", 0.0)),
            m.tick,
        )
...
function _view_agents_detail(m::KaimonModel, area::Rect, buf::Buffer, ags)
    ...
    y <= bottom(inner) && (
        set_string!(buf, inner.x + 1, y, "─"^max(0, inner.width - 2), tstyle(:border));
        y += 1
    )
    feed_top = y
    ...
    for row = 1:avail
        ...
        if idx == sel
            ...
            set_string!(
                buf,
                inner.x + 1 + length(age) + length(kind),
                y,
                _oneline(ev.summary),
                tstyle(:text);
                max_x = max_cx,
    ...
    off + avail < nev &&
        set_string!(buf, right(inner), feed_top + avail - 1, "▼", tstyle(:text_dim))
end
...
function _view_agent_event_popup(m::KaimonModel, area::Rect, buf::Buffer)
    ...
    for r = area.y:bottom(area), c = area.x:right(area)   # dim the tab behind it
    ...
    for r = inner.y:bottom(inner), c = inner.x:right(inner)   # clear interior
...
function _selected_agent_id(m::KaimonModel, ags)
    (isempty(ags) || m.agentmon_selected < 1 || m.agentmon_selected > length(ags)) &&
        return ""
...
function _open_event_popup!(m::KaimonModel)
    ...
    m.agentmon_popup_pane = ScrollPane(
        String.(split(body, '\n'));
        word_wrap = true,
        following = false,
        show_scrollbar = true,
...
function _handle_agents_nav!(m::KaimonModel, evt::KeyEvent, fp::Int)
    ...
    if fp == 2
        ...
        @match evt.key begin
            ...
            :down => (
                m.agentmon_selected = min(max(1, n), m.agentmon_selected + 1);
                m.agentmon_scroll = 0;
                m.agentmon_event_sel = 0
            )
            _ => nothing
...
function _open_agent_history!(m::KaimonModel, id::AbstractString)
    ...
    m.agentmon_history_pane =
        MarkdownPane(_agent_history_markdown(id); show_scrollbar = true)
