...
const agent_open_tool = @mcp_tool :agent_open "Spawn a Kaimon-owned AI agent (headless claude) in a directory. Returns the agent_id. Events stream on the gate event bus channel 'agent:<id>' as {kind,turn,data} JSON." Dict(
    ...
    "properties" => Dict(
        ...
        "model" => Dict("type" => "string", "description" => "Model alias or id (default claude-sonnet-4-6). Prefix with 'ollama:<tag>' for a local Ollama model, or 'vmlx:<tag>' for a local MLX model via vmlx (Apple Silicon, default host :8000)."),
