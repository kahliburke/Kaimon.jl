...
const agent_open_tool = @mcp_tool :agent_open "Spawn a Kaimon-owned AI agent in a directory and return its agent_id. The agent is chosen by 'model': the headless claude CLI by default, an Agent Client Protocol agent ('acp:...'), or a local model ('ollama:...'/'vmlx:...'). Events stream on the gate event bus channel 'agent:<id>' as {kind,turn,data} JSON." Dict(
    "type" => "object",
    "properties" => Dict(
        "cwd" => Dict("type" => "string", "description" => "Working directory for the agent (must exist)."),
        "model" => Dict("type" => "string", "description" => "Model alias or id (default 'sonnet' — the family alias, which resolves to the CLI's latest Sonnet; pass a pinned id like 'claude-sonnet-5' for reproducibility). Optional inline effort suffix for Claude models: 'claude-sonnet-5/high' or 'sonnet/low' (an explicit 'effort' arg overrides it). Prefix with 'ollama:<tag>' for a local Ollama model, or 'vmlx:<tag>' for a local MLX model via vmlx (Apple Silicon, default host :8000). Prefix with 'acp:<agent>[:<model>]' to drive an Agent Client Protocol agent instead of the claude CLI — known agents are opencode, claude, gemini (e.g. 'acp:opencode:opencode/minimax-m3'); note that a preset's deny list reaches an ACP agent's OWN file and shell tools only for opencode and claude, and the spawn reports on its event stream when it does not."),
