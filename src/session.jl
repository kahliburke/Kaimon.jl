...
function get_server_instructions()
    ...
    • grep_code(pattern="…") finds an EXACT pattern/regex over the live working tree — every occurrence, with its enclosing symbol. Use it for a symbol name, a call site, a string, or a TODO; add query="…" to also rank the matching files by relevance, or no_ignore=true to also search logs and generated/gitignored files. path=/file=/glob= are all PROJECT-ROOT-relative (a glob is anchored to the repo root like `cd repo && rg -g '<glob>'`, NOT to path=), so don't repeat a path= prefix inside a glob — a bare basename glob (no `/`) matches at any depth.
