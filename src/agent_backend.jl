...
# Launch a Windows shim CLI (npm installs `claude` as a `.cmd`, plus a `.ps1`) through its
# interpreter — CreateProcess can't exec those directly. Shared cross-platform logic lives in
# Utils.launch_argv; `_spawn_argv` keeps the `iswin`-injectable signature the agent tests use.
