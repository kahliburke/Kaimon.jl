...
function create_handler(
    ...
    return function handle_request(req::HTTP.Request)
        # PRECONDITION: this closure is invoked ONLY by _hybrid_handler_impl, and
        # only after it has run the origin check, the API-key/nonce/IP security
        # gate, and the static-endpoint handlers (AGENTS.md, /vscode-response,
        # GET-SSE, DELETE, MCP-Protocol-Version, empty-body) via the _stream_*
        # helpers. So the request here has already passed security and is a
        # non-empty request none of those static branches claimed — this handler
        # therefore covers only the OAuth POST endpoints and JSON-RPC method
        # dispatch. (It stays Request-based so the caller can convert the returned
        # Response and apply its notification-flush SSE upgrade uniformly.)
...
function start_mcp_server(
    ...
    function _hybrid_handler_impl(
        ...
        try
            # Static endpoints + protocol/empty-body checks (mcp_stream_handlers.jl).
            _stream_agents_md(http, req) && return nothing
            _stream_vscode_response(http, req, body) && return nothing
            _stream_connect_tcp(http, req, body) && return nothing
            _stream_get_sse(http, req) && return nothing
            _stream_delete(http, req) && return nothing
            _stream_protocol_version(http, req) && return nothing
            _stream_empty_body(http, body) && return nothing
...
# Streaming (HTTP.Stream) origin/security gate + static-endpoint helpers,
# extracted from _hybrid_handler_impl. Same call-time-resolution property.
include("mcp_stream_handlers.jl")
